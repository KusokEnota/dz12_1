...
objects = ['Английский', 'Математика', 'Химия', 'Черчение', 'ДПЮ']
file = 'objects.csv'
...
class Valid:
    ...
    def validate(self, value):
        for param in self.predicate:
            if not param(value):
                raise ValueError('ФИО с большой буквы + Кириллица!')


class Student:
    MAX_TEST = 100

    name = Valid(str.istitle, str.isalpha)
    surname = Valid(str.istitle, str.isalpha)
    patronymic = Valid(str.istitle, str.isalpha)
    ...
    def info(self):
        ...
        if self._upgrade:
            for key, value in self._journal.items():
                ...
                print(
                    f"Средний балл тестов по предмету {key} : {round(sum(map(int, value['tests'])) / len(value['tests']))}")
...
if __name__ == '__main__':
    stud_1 = Student('Вр', 'Денис', 'Владимирович')
    print(stud_1)
    stud_1.fill_journal()
    stud_1.info()